...
def _progress_md():
    return Path(os.environ.get('TUTOR_PROGRESS', HERE / 'PROGRESS.md'))
...
def _tree_lines(con):
    ...
    rows = con.execute(
        ...
        " source FROM concepts WHERE parked = 0 ORDER BY depth, slug").fetchall()
    pid = detect_project_id()
    frames = stack.live(con, pid)
    parked = con.execute("SELECT COUNT(*) FROM concepts WHERE parked=1").fetchone()[0]
    total = len(rows)
    ...
    L.append("")
    L.append("STACK  (holes nest; only the deepest one is teachable)")
    if frames:
        L.append(f"    {' > '.join(f['slug'] for f in frames)}")
        for f in frames:
            got = stack.rungs(con, f['id'])
            pend = stack.pending(con, f['id'])
            bar = ' '.join(f"{r[:4]}={got.get(r) or '-'}" for r in stack.RUNGS)
            tip = '  <-- teaching this' if f['state'] == 'ACTIVE' else ''
            L.append(f"      {'  ' * f['depth']}{f['slug']:<20} {bar}{tip}")
            if pend:
                L.append(f"      {'  ' * f['depth']}  pending: {', '.join(pend)}")
    else:
        L.append("    empty -- nothing is being taught right now.")
    L.append("")
    ...
    for d in sorted({r['depth'] for r in rows}):
        ...
        for r in band:
            ...
            # [h] = born from a hole in his own answer, not scraped from the
            # codebase. Those are the rungs the ladder did not predict.
            h = '[h] ' if r['source'] == 'hole' else '    '
            L.append(f"      {g} {h}{r['slug']:<26}{tail}".rstrip())
        L.append("")
    L.append("  [x] you own it   [!] probed and missed   [-] probed   [ ] untouched")
    L.append("  [h] found as a hole in your own answer, not scraped from the code")
    L.append("")
...
def tree(args=None):
    ...
    if getattr(args, 'quiet', False):
        ...
        return
    print('\n'.join(lines))
    print(f"\n(also written to {dest} -- keep that file open in a tab)")
...
# The map only moves when the stack or the phase moves. v3 rewrote PROGRESS.md
# after all twenty-odd write commands, which cost a file write on every `ask`.
TREE_ON = ('classify', 'pass', 'phase', 'session')
