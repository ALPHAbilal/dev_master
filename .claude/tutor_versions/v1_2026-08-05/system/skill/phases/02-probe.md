...
`UNKNOWN → SEEN` on a hit during sweep. `→ EXPLAINED` only on blank-page recall
with no file open. `→ APPLIED` only through a gate (PHASE 4), `evidence` must be
`unaided`. `assisted` never reaches APPLIED, however well he followed.
Demote as readily as promote.

Write every question through the helper — it stamps the axis and the clock, and
increments `attempts`/`fails` for you:

```
python3 .claude/tutor/tutor_db.py probe <slug> SWEEP <faculty> HIT|MISS|PARTIAL \
    -q "<what you asked>" -a "<what he said>" --seconds <N>
```
...
`--seconds` is how long he took. Skip it if you did not track it, but a profile
with no clock cannot answer whether any of this is getting faster.
