...
```
python3 .Codex/tutor/tutor_db.py session "<model>" "<what this session is for>"
python3 .Codex/tutor/tutor_db.py brief
```
...
`phase-guard` hook (`.Codex/tutor/tutor_hook.py`) — which reads the DB's real
...
lives under `.Codex/skills/tutor_v3/phases/machinery/`.
