...
1. Draw the defect from a concept already in the bank so the probe attaches to
   something. Prefer concepts at `state = CAN` — this tests whether the
...
5. Record with the correct faculty:
   `ask <slug>` when you show the broken code, then
   `probe <slug> BREAK debug HIT|MISS|PARTIAL` when he answers.
