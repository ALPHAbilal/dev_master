...
Invoked by `/tutor transfer <slug>`. Requires the concept to be `CAN`.

## Why this phase exists

`CAN` means he produced it unaided, once, in one place. That is real, and it
...
1. Pick a `CAN` concept whose `next_review` is due, or one the port touches.
...
4. Record `probe <slug> TRANSFER write HIT` and then:
   `promote <slug> CAN --evidence transferred --copy-checked`
   This refuses unless the concept is already `CAN` and a TRANSFER-phase HIT
   exists.
5. On a MISS: `demote`. The `CAN` was a memory of one file, and now you know.
   That is a finding, not a setback — say so plainly and move on.

## Rules

- Never run transfer on the same day the concept reached `CAN`. Time is part
